...
from uvpipx.exceptions import VenvNotFoundError, VenvNotReadyError
...
def uvpipx_load_venv(
    ...
    _, venv = uvpipx_venv_factory(package_name, name_override)  # venv_m unused

    if not venv.exists():
        raise VenvNotFoundError(package_name, venv.venv_path)

    if not (venv.venv_path / "uvpipx.json").exists():
        raise VenvNotReadyError(venv.venv_path, "uvpipx.json configuration file")
    ...
    if current_vers_dict is None:
        raise VenvNotReadyError(venv.venv_path, "invalid or corrupted configuration")
